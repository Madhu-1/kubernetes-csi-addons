...
func (f *Framework) CreatePVC(name string, size string, storageClass string) *corev1.PersistentVolumeClaim {
	ginkgo.By(fmt.Sprintf("Creating PVC %s with size %s", name, size))
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to create PVC")
...
func (f *Framework) CreateBlockPVC(name string, size string, storageClass string) *corev1.PersistentVolumeClaim {
	ginkgo.By(fmt.Sprintf("Creating Block mode PVC %s with size %s", name, size))
	...
	err := f.Client.Create(ctx, pvc)
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to create Block mode PVC")
...
func (f *Framework) WaitForPVCBound(pvcName string) *corev1.PersistentVolumeClaim {
	ginkgo.By(fmt.Sprintf("Waiting for PVC %s to be bound", pvcName))
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "PVC did not become bound in time")
	return pvc
}
...
func (f *Framework) GetPVC(name string) *corev1.PersistentVolumeClaim {
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to get PVC")
	return pvc
}
...
func (f *Framework) GetPV(name string) *corev1.PersistentVolume {
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to get PV")
	return pv
}

// DeletePVC deletes a PersistentVolumeClaim
func (f *Framework) DeletePVC(name string) {
	ginkgo.By(fmt.Sprintf("Deleting PVC %s", name))
	...
	if err != nil && !apierrors.IsNotFound(err) {
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to delete PVC")
	}
}

// CreatePod creates a pod that uses a PVC
func (f *Framework) CreatePod(name string, pvcName string, command []string) *corev1.Pod {
	ginkgo.By(fmt.Sprintf("Creating pod %s with PVC %s", name, pvcName))
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to create pod")
...
func (f *Framework) CreatePodWithBlockDevice(name string, pvcName string, command []string) *corev1.Pod {
	ginkgo.By(fmt.Sprintf("Creating pod %s with block device PVC %s", name, pvcName))
	...
	err := f.Client.Create(ctx, pod)
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to create pod with block device")
...
func (f *Framework) WaitForPodRunning(podName string) *corev1.Pod {
	ginkgo.By(fmt.Sprintf("Waiting for pod %s to be running", podName))
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Pod did not become running in time")
	return pod
}

// DeletePod deletes a pod
func (f *Framework) DeletePod(name string) {
	ginkgo.By(fmt.Sprintf("Deleting pod %s", name))
...
func (f *Framework) WaitForPodDeleted(podName string) {
	ginkgo.By(fmt.Sprintf("Waiting for pod %s to be deleted", podName))
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Pod was not deleted in time")
}
...
func (f *Framework) GetStorageClass(name string) *storagev1.StorageClass {
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to get storage class")
	return sc
}
...
func (f *Framework) ListStorageClasses() *storagev1.StorageClassList {
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to list storage classes")
	return scList
}

// CreateSecret creates a secret
func (f *Framework) CreateSecret(name string, data map[string][]byte) *corev1.Secret {
	ginkgo.By(fmt.Sprintf("Creating secret %s", name))
