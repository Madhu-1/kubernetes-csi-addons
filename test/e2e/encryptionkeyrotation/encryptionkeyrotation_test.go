...
var _ = ginkgo.Describe("EncryptionKeyRotation", func() {
	var (
	...
	ginkgo.Context("EncryptionKeyRotationJob", func() {
		ginkgo.Context("Filesystem mode PVC", func() {
			ginkgo.It("should successfully rotate encryption key on a filesystem PVC", func() {
				ginkgo.By("Creating a filesystem PVC with encryption enabled")
				pvc := f.CreatePVC("test-pvc-fs-encrypted", "", f.GetEncryptionKeyRotationStorageClassName())
				pvc = f.WaitForPVCBound(pvc.Name)
				gomega.Expect(pvc.Status.Phase).To(gomega.Equal(corev1.ClaimBound))

				ginkgo.By("Creating an EncryptionKeyRotationJob")
				...
				gomega.Expect(ekrJob.Status.Message).To(gomega.ContainSubstring("successfully"))
			})
		...
		ginkgo.Context("Block mode PVC", func() {
			ginkgo.It("should successfully rotate encryption key on a block PVC", func() {
				ginkgo.By("Creating a block mode PVC with encryption enabled")
				pvc := f.CreateBlockPVC("test-pvc-block-encrypted", "", f.GetEncryptionKeyRotationStorageClassName())
				pvc = f.WaitForPVCBound(pvc.Name)
				gomega.Expect(pvc.Status.Phase).To(gomega.Equal(corev1.ClaimBound))
